...
# Zet SERIAL_PORT op bv. "COM3" als auto-detectie niet werkt.
# Laat op None staan om automatisch een poort te zoeken.
# ======================================================
SERIAL_PORT = None
BAUDRATE = 115200
...
def ensure_serial_modules():
    try:
        ...
        print(f"pyserial nog niet beschikbaar: {first_error}")

    if not AUTO_INSTALL_PYSERIAL:
        raise RuntimeError(
            "pyserial is niet geïnstalleerd. Zet AUTO_INSTALL_PYSERIAL op True of installeer pyserial manueel."
        )
    ...
    for python_exe in python_candidates:
        try:
            print(f"Probeer pyserial te installeren via: {python_exe}")
            ...
            serial = importlib.import_module("serial")
            list_ports = importlib.import_module("serial.tools.list_ports")
            print("pyserial is geïnstalleerd.")
            return serial, list_ports
        except Exception as install_error:
            last_error = install_error
            print(f"Installatie mislukt via {python_exe}: {install_error}")

    raise RuntimeError(
        "Kon pyserial niet automatisch installeren.\n"
        "Open Blender's Python console of terminal en installeer pyserial.\n"
...
def stop_previous_listener():
    ...
    print("Vorige Blender serial listener gestopt.")
...
def choose_serial_port():
    ...
    if SERIAL_PORT:
        for port in available_ports:
            if port.device == SERIAL_PORT:
                print(f"Seriële poort uit script gebruikt: {SERIAL_PORT}")
                return SERIAL_PORT
        raise RuntimeError(f"De ingestelde serial poort bestaat niet: {SERIAL_PORT}")

    if not available_ports:
        raise RuntimeError("Geen seriële poorten gevonden. Verbind eerst de Arduino.")
    ...
    if len(preferred_ports) == 1:
        print(f"Automatisch Arduino-poort gekozen: {preferred_ports[0].device}")
        return preferred_ports[0].device

    if len(available_ports) == 1:
        print(f"Automatisch enige poort gekozen: {available_ports[0].device}")
        return available_ports[0].device

    print("Meerdere poorten gevonden:")
    for port in available_ports:
        print(f"- {port.device}: {port.description}")

    first_port = preferred_ports[0] if preferred_ports else available_ports[0]
    print(f"Eerste bruikbare poort gekozen: {first_port.device}")
    print("Zet SERIAL_PORT bovenaan expliciet vast als Blender de verkeerde poort pakt.")
...
def apply_angle_to_bone(pose_bone, axis, angle_deg, invert=False, offset_deg=0.0):
    ...
    # Basis apart behandelen: quaternion voorkomt flip/terugspringen rond 180°.
    if pose_bone.name == BONES["basis"]["bone_name"]:
        ...
        if axis == "X":
            ...
            raise ValueError(f"Onbekende rotatie-as: {axis}")
...
def start_listener():
    ...
    try:
        ...
        print(f"Kon seriële poort niet openen: {port_name}")
        print(f"Fout: {exc}")
        print("Sluit de Arduino Serial Monitor / Plotter.")
        return

    time.sleep(2.0)

    print("Blender serial listener gestart.")
    print(f"Poort: {port_name} @ {BAUDRATE}")
    print(f"Armature: {ARMATURE_NAME}")
    print("Verwacht CSV: basis,main,fore,wrist")
    ...
    def timer_callback():
        try:
            ...
            print(f"Fout in Blender serial listener: {exc}")
            try:
                if ser.is_open:
                    ser.close()
            except Exception:
